...
func _init() -> void:
	for realm_index in [0, 1, 2]:
		for deck_name in DECKS.keys():
			print(_run_case(str(deck_name), DECKS[deck_name], int(realm_index), 60))
	quit(0)

func _run_case(deck_name: String, deck_ids: Array, realm_index: int, runs: int) -> String:
	var wins := 0
	var hp_total := 0
	...
	for i in range(runs):
		var hp := 35
		var max_hp := 35
		var deck: Array = deck_ids.duplicate()
		var reserve: Array = []
		var failed_stage := 0
		for stage_index in range(ROUTE.size()):
			var node: Dictionary = ROUTE[stage_index]
			var result: Dictionary = _run_battle(deck, reserve, hp, max_hp, int(node.get("battle_number", 1)), str(node.get("type", "normal")), realm_index)
	...
	var avg_fail := 0.0 if losses <= 0 else float(fail_stage_total) / float(losses)
	return "realm_index %d | %s | win %d/%d (%.1f%%) | avg_win_hp %.1f | avg_fail_stage %.1f" % [
		realm_index,
		deck_name,
		wins,
		runs,
...
func _run_battle(deck_ids: Array, reserve_ids: Array, hp: int, max_hp: int, battle_number: int, encounter_type: String, realm_index: int) -> Dictionary:
	...
	if battle.phase == "reward" and not battle.reward_options.is_empty():
		battle.choose_reward(str(battle.reward_options[0]))
	return {
...
func _play_card(battle) -> bool:
	for card_id in ["青锋剑", "护身符", "回春符", "小还丹", "起剑诀", "引剑入体", "疾剑诀", "雷击符", "火球符", "金刃符"]:
