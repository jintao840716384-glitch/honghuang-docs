...
static func text(text_id: String, language_id: String, params: Dictionary = {}, source_fallback := "") -> String:
	var value := LocalizationDatabaseScript.text(text_id, language_id, source_fallback)
	return format_text(value, params)

static func text_for_settings(text_id: String, settings: Dictionary, params: Dictionary = {}, source_fallback := "") -> String:
	return text(text_id, language_from_settings(settings), params, source_fallback)
